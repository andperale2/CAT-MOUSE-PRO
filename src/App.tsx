...
import AnimeLiveActionGuide from './components/AnimeLiveActionGuide';

export default function App() {
  // Navigation active modules
  type TabType = 'guide' | 'overlays' | 'profiles' | 'shizuku' | 'vehicles' | 'devices' | 'presets';
      ...
      <main className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0 relative bg-[#09090b]">
        ...
        <div className="lg:col-span-8 flex flex-col overflow-y-auto p-5 md:p-6 pb-20 space-y-6">
          ...
          <div className="bg-[#111114] border border-white/5 p-1 rounded-2xl grid grid-cols-3 md:grid-cols-7 gap-1 select-none shadow-xl">
            {(['guide', 'overlays', 'profiles', 'shizuku', 'vehicles', 'devices', 'presets'] as const).map((tab) => {
              const labels = {
                guide: 'Guía Anime IA',
                overlays: 'Overlays',
                profiles: 'Playbook',
              ...
              const icons = {
                guide: <HelpCircle className="w-4 h-4 text-purple-400" />,
          ...
          <div className="relative flex-1">
            ...
            {/* VIEW 0: GUIA ANIME TO LIVE ACTION WITH VIRTUAL PROFESSOR */}
            {activeTab === 'guide' && (
              <AnimeLiveActionGuide
                onAddLog={(source, level, msg) => handleAddLog(source as any, level as any, msg)}
              />
            )}
